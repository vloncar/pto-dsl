...
AICORE inline void pack_vec_subtile(TileDst &dst, TileSrc &src)
{
    constexpr unsigned kBlockSizeElem = BLOCK_BYTE_SIZE / sizeof(typename TileDst::DType);
    TCopy<TileDst, TileSrc, kBlockSizeElem, TileSrc::RowStride, TileDst::RowStride>(
...
template <typename TileDataF_T, typename ReduceTileF_T, typename TileDataH_T, typename TileDataHPush_T,
          typename TileOutT, std::size_t SrcBuffers, std::size_t XexpBuffers, std::size_t PushBuffers,
...
AICORE inline void allocate_vec_tile_buffers(TileDataF_T (&srcTiles)[SrcBuffers], ReduceTileF_T &m1_local_max,
                                             ...
                                             TileDataH_T (&x_expT)[XexpBuffers],
                                             TileDataHPush_T (&x_expPushT)[PushBuffers],
                                             ...
                                             TileDataF_T &triu)
{
    ...
    constexpr std::size_t push_xexp_bytes = tile_buffer_total_bytes<TileDataHPush_T, PushBuffers>();
    ...
    static_assert(push_xexp_bytes <= float_tile_bytes, "Push tiles must fit in the triu workspace");
    ...
    const uint32_t triu_offset = offset;
    TASSIGN(triu, triu_offset);
    for (std::size_t idx = 0; idx < PushBuffers; ++idx) {
        TASSIGN(x_expPushT[idx], triu_offset + static_cast<uint32_t>(idx * tile_storage_bytes<TileDataHPush_T>()));
    }
...
template <int HEAD_SIZE, int CUBE_S0, int CUBE_S1, int TILE_S1, int QKP_CV_FIFO, int CV_FIFO_CONS_SYNC_PERIOD,
          ...
          typename TileDataHPush_T, typename ReduceTileF_T, typename TSyncQK2SM, typename PPipe,
          std::size_t PushBuffers>
AICORE inline void compute_p(int tile_id, int row_slice, __gm__ float *qk_tile_fifo, __gm__ float *exp_max_ififo,
                             ...
                             TileDataH_T &x_expT, TileDataHPush_T (&x_expPushT)[PushBuffers],
                             ...
                             TSyncQK2SM &qk2smSync, PPipe &pPipe, int blk_idx)
{
    ...
    static_assert(PushBuffers >= kTileFactor, "Need one push buffer per TILE_S1 subtile");
    if constexpr (DAV_VEC) {
        ...
        using TileDataH_PushSlice = Tile<TileType::Vec, half, Vec_S0, Cube_S1, BLayout::RowMajor, Vec_S0, Cube_S1>;
        const uint64_t push_row_byte_offset =
            static_cast<uint64_t>(row_slice * Vec_S0 * Cube_S1 * sizeof(typename TileDataH_T::DType));
        for (int sub_col = 0; sub_col < static_cast<int>(kTileFactor); ++sub_col) {
            ...
            TileDataH_PushSlice xExpPushSlice;
            TASSIGN(xExpPushSlice, (uint64_t)x_expPushT[sub_col].data() + push_row_byte_offset);
            pack_vec_subtile(xExpPushSlice, xExpSub);

            if (row_slice == static_cast<int>(kTileFactor) - 1) {
                TPUSH<PPipe, TileDataHPush_T, TileSplitAxis::TILE_UP_DOWN>(pPipe, x_expPushT[sub_col]);
            }
        }
                              ...
                              __gm__ uint8_t *profile_buf, int s0, int s1)
{
    ...
    constexpr uint32_t SubblockRows = Cube_S0 / VEC_CORES;
    using TileDataHPush_T =
        Tile<TileType::Vec, half, SubblockRows, Cube_S1, BLayout::RowMajor, SubblockRows, Cube_S1>;
    ...
    TileDataHPush_T x_expPushT[kTileFactor];
    ...
    allocate_vec_tile_buffers<TileDataF_T, ReduceTileF_T, TileDataH_T, TileDataHPush_T, TileOutGuT, srcVecTNBuffers,
                              xexpVecTNBuffers, kTileFactor, outOTileNBuffers>(
        qkVecTile, m1_local_max, input_reduce_tmp, l1_local_sum, m2_global_max, l2_global_sum, l1_exp_max_ififo,
        x_expT, x_expPushT, pvVecTile, runningOTile, triu);
            ...
            if constexpr (DAV_VEC) {
                if (next_qk_tile != -1) {
                    compute_p<HEAD_SIZE, CUBE_S0, CUBE_S1, Tile_S1, qkp_tile_fifo_size, CV_FIFO_CONS_SYNC_PERIOD,
                        ...
                        x_expT[p_gu_src_pingpong_id % xexpVecTNBuffers], x_expPushT, input_reduce_tmp, m1_local_max,
