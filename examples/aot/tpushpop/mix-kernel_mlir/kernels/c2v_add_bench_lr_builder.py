...
def meta_data():
    ...
    y_half_view_ty = pto.SubTensorType(shape=[M, N // 2], dtype=dtype)
    ...
    vec_ty = pto.TileBufType(shape=[M, N // 2], dtype=dtype, memory_space="VEC")
    return locals()
...
def module():
    ...
    def cube_kernel(gm_slot_buffer: "ptr_ty", gm_x: "ptr_ty") -> None:
        ...
        for tile_idx in pto.range(c0, c_num_tiles, c1):
            ...
            # TileSplitAxis::TILE_LEFT_RIGHT is split=2 in PTOAS.
            pto.tpush_to_aiv(acc_tile, 2)
    ...
    def vector_kernel(gm_slot_buffer: "ptr_ty", gm_y: "ptr_ty") -> None:
        ...
        for tile_idx in pto.range(c0, c_num_tiles, c1):
            gm_y_tile_view = pto.slice_view(
                y_half_view_ty,
                source=gm_y_tensor,
                offsets=[tile_idx, c0, col_offset],
                sizes=[c1, c16, c8],
            )

            recv_tile = pto.tpop_from_aic(vec_ty, 2)
            tile.add(recv_tile, recv_tile, doubled_tile)
            pto.store(doubled_tile, gm_y_tile_view)
            pto.tfree_from_aic(2)
